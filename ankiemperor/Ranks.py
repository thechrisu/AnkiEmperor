...
class Ranks(object):
    ...
    def updateRank(self, currentGold, constrCompletePercentage, isInitialCheck):
        newGoldRankPos = self.__getRank(
            self.__goldRanks, currentGold, self.__currentGoldRankPos
        )
        newConstrRankPos = self.__getRank(
            self.__constructionRanks,
            constrCompletePercentage,
            self.__currentConstrRankPos,
        )

        # If a rank has changed
        if (newGoldRankPos != self.__currentGoldRankPos) or (
            newConstrRankPos != self.__currentConstrRankPos
        ):
        ...
        self.__currentGoldRankPos = self.__getRank(
            self.__goldRanks, currentGold, self.__currentGoldRankPos
        )
        self.__currentConstrRankPos = self.__getRank(
            self.__constructionRanks,
            constrCompletePercentage,
            self.__currentConstrRankPos,
        )
    ...
    def __getRank(self, ranks, rankToCheck, currentRankPos):
        ...
        while (currentRankPos < len(ranks) - 1) and (
            rankToCheck >= ranks[currentRankPos + 1]["rankLimit"]
        ):
            currentRankPos += 1
