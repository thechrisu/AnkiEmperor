...
class EventManager(object):
    ...
    def createConstructionEvent(self, finishedObject):
        self.__events.appendleft(
            ConstructionEvent(self.ae, finishedObject, self.__options)
        )
    ...
    def checkStatsEvent(self, stats):
        ...
        if (stats.getCardsAnsweredToday() % 50 == 0) and (
            0 < stats.getCardsAnsweredToday() <= 250
        ):
            self.__events.append(
                CardsAnsweredStatsEvent(
                    self.ae,
                    stats.getCardsAnsweredToday(),
...
class ConstructionEvent(Event):
    ...
    def performEventAndGetText(self):
        ...
        txt = (
            '<table cellpadding="2"><tr><td><img src="%s" height=60></td>'
            % getImagePath(self.__finishedObject.cityName, self.__finishedObject.image)
        )
        txt += (
            '<td><p>Your empire is growing!<br>Your people have completed the following construction:<br><span style="font-size:xx-large; font-weight:bold">%s</span></p></td></tr></table>'
            % self.__finishedObject.name
        )
...
class RankEvent(Event):
    ...
    def performEventAndGetText(self):
        self.playMajorEventSound()
        txt = '<table cellpadding="2"><tr><td><img src="%s" height=60></td>' % getIcon(
            RANK_UP_ICON
        )
        txt += (
            '<td><p>Your empire is flourishing!<br>You have been given a <strong>new title</strong> by your people:</p><p style="font-size:x-large; font-weight:bold">%s</p></td></tr></table>'
            % self.__newRank
        )
...
class CardsAnsweredStatsEvent(Event):
    ...
    def performEventAndGetText(self):
        ...
        txt += (
            "<td><p>Great work, you have answered <strong>%s cards</strong> today!<br><strong>%s gold</strong> extra for you, now keep going!</td></tr></table>"
            % (self.__cardsAnswered, self.__cardsAnswered / 2)
        )
...
class CardsRecordStatsEvent(Event):
    ...
    def performEventAndGetText(self):
        ...
        txt += (
            "<td><p>New record! You have answered <strong>%s cards</strong> today, more than ever before!<br> You are awesome, <strong>50 gold</strong> extra for you!</td></tr></table>"
            % (self.__cardsAnswered)
        )
