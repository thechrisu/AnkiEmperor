...
class DBConnect(object):
    ...
    def getAllObjects(self):
        self.c.execute(
            "select o.*, c.* from objects o, cities c where o.cityid = c.cityID order by o.price"
        )
    ...
    def updateObject(self, object):
        self.c.execute(
            "update objects set state = ? where objectid = ?",
            (object.getRemainingRounds(), object.objectID),
        )
        self.conn.commit()
    ...
    def getOptions(self, deckname):
        self.c.execute(
            "select option, optionDesc, value from options where deckname is ?",
            [deckname],
        )
        result = self.c.fetchall()
        return result
    ...
    def setOption(self, deckname, option):
        self.c.execute(
            "insert or replace into options(optionID, deckname, option, optionDesc, value) values((SELECT optionID FROM options WHERE deckname is ? AND option is ?), ?, ?, ?, ?)",
            (
                deckname,
                option.name,
                deckname,
    ...
    def getRanks(self, type):
        self.c.execute(
            "select * from ranks where type = ? order by rankLimit ASC", [type]
        )
        result = self.c.fetchall()
    ...
    def saveStats(self, stats):
        self.c.execute(
            "insert or replace into stats(statsID, statsDay, cardsAnswered) values((SELECT statsID FROM stats WHERE statsDay is ?), ?, ?)",
            (stats["statsDay"], stats["statsDay"], stats["cardsAnswered"]),
        )
        self.conn.commit()
