...
class CitySelectView(object):
    ...
    def selection(self):
        ...
        html += '<td>Current Gold:</td><td>%s <img src="file:///%s"></td></tr>' % (
            self.treasureChest.getCurrentGold(),
            getIcon(GOLD_ICON),
        )
        html += (
            '<tr valign="bottom"><td>Constructed:</td><td>%s <img src="file:///%s"></td></tr></table>'
            % (completedConstructions, getIcon(CONSTR_ICON))
        )
        ...
        for city in sorted(country.getCities(), key=attrgetter("price")):
            ...
            if city.isUnlocked():
                ...
                constructedhtml += (
                    '<a href="CitySelectView||construction||%s||%s"><strong style="font-size:x-large;">%s</strong></a>'
                    % (country.getName(), city.getName(), city.getName())
                )
                constructedhtml += '</td><td align="right">'

                if city.getCompletedObjectsCount() == city.getTotalObjectsCount():
                    constructedhtml += '<span style="color:green">%i / %i</span>' % (
                        city.getCompletedObjectsCount(),
                        city.getTotalObjectsCount(),
                    )
                else:
                    constructedhtml += "%i / %i" % (
                        city.getCompletedObjectsCount(),
                ...
                goldAndConstructionColors = self.__getGoldAndConstructionColor(
                    city, self.treasureChest.getCurrentGold(), completedConstructions
                )
                unlockhtml += "<tr><td>%s</strong> %s</td>" % (
                    city.getName(),
                    capitalIcon,
                )
                unlockhtml += (
                    '<td width="70"><font color="%s">%s</font> <img src="file:///%s"><br />'
                    % (
                        goldAndConstructionColors[1],
                        city.getConstructionsNeeded(),
                        getIcon(CONSTR_ICON),
                    )
                )
                unlockhtml += (
                    '<font color="%s">%s</font> <img src="file:///%s"></td>'
                    % (
                        goldAndConstructionColors[0],
                        city.getPrice(),
                        getIcon(GOLD_ICON),
                    )
                )
                ...
                if (
                    city.getPrice() <= self.treasureChest.getCurrentGold()
                    and self.__allCitiesUnlockedIfCapital(country, city)
                    and city.getConstructionsNeeded() <= completedConstructions
                ):
                    unlockhtml += (
                        '<td width="30"><a href="CitySelectView||unlockPopup||%s||%s">Unlock</a></td>'
                        % (country.getName(), city.getName())
                    )
                else:
                    unlockhtml += (
                        '<td width="30"><font color="gray">Unlock</font></td></tr>'
                    )
    ...
    def __getCitySelectionLegend(self):
        ...
        html += (
            '<tr><td><img src="file:///%s"></td><td>Capital city: All other cities in this country need to be unlocked first.</td></tr>'
            % getIcon(STAR_ICON)
        )
        html += (
            '<tr><td><img src="file:///%s"></td><td>Required number of completed constructions in this country.</td></tr>'
            % getIcon(CONSTR_ICON)
        )
        html += (
            '<tr><td><img src="file:///%s"></td><td>Unlock costs in gold.</td></tr></table>'
            % getIcon(GOLD_ICON)
        )
    ...
    def unlockPopup(self, countryName, cityName):
        ...
        msgBox.setText(
            "Do you want to unlock %s for %s Gold?" % (city.getName(), city.getPrice())
        )
    ...
    def construction(self, countryName, cityName):
        ...
        html += '<p>Current Gold: %s <img src="file:///%s"></p>' % (
            self.treasureChest.getCurrentGold(),
            getIcon(GOLD_ICON),
        )
        ...
        if city.getUnbuiltObjects(False):
            ...
            for unbuilt in city.getUnbuiltObjects(False):
                ...
                if unbuiltCounter < 5:
                    ...
                    html += (
                        '<td width="60" align="center" rowspan="2"><img src="file:///%s" height="40"></td>'
                        % getImagePath(unbuilt.cityName, unbuilt.image)
                    )
                    html += (
                        '<td valign="middle" rowspan="2"><a href="%s" %s>%s</a></td>'
                        % (unbuilt.link, getLinkColor(), unbuilt.name)
                    )
                    html += (
                        '<td width="70" valign="bottom" height="50%%">%s <img src="file:///%s"></td>'
                        % (unbuilt.price, getIcon(GOLD_ICON))
                    )
                    html += '<td width="30" valign="middle" rowspan="2">'

                    if unbuilt.price <= self.treasureChest.getCurrentGold():
                        html += (
                            '<a href="ae||getBuildingAuthority||buildObject||%s">Build</a>'
                            % unbuilt.objectID
                        )
                    else:
                    ...
                    html += "</td></tr>"
                    html += (
                        '<tr bgcolor="%s"><td valign="bottom" height="50%%">%s <img src="file:///%s"></td></tr>'
                        % (
                            bgColors[unbuiltCounter % 2],
                            unbuilt.getRounds(),
                            getIcon(ROUND_ICON),
                        )
                    )
            ...
            if unbuiltCounter == 6:
                html += (
                    "<p>There are more constructions in %s. Only the five cheapest ones are displayed.</p>"
                    % cityName
                )
        else:
