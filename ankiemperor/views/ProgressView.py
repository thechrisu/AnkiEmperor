...
class ProgressView(object):
    ...
    def overview(self):
        ...
        html += "<h2>Gold</h2>"
        html += '<p>Total gold earned: %s <img src="file:///%s"></p>' % (
        ...
        for country in self.buildingAuthority.getCountries():
            ...
            html += "<p>You have completed %s of %s constructions.</p>" % (
                country.getCompletedObjectsCount(),
                country.getTotalObjectsCount(),
            )
            html += self._getProgressBar(country.getCompletedObjectsPercentage())
            html += (
                '<p><a href="ProgressView||countryProgress||%s">Click here</a> for more details.</p>'
                % country.getName()
            )
    ...
    def countryProgress(self, countryName):
        ...
        for city in country.getCities():
            builtObjectCount = len(city.getCityObjects()) - len(
                city.getUnbuiltObjects(True)
            )
            html += (
                '<table><tr><td><h3><a href="ProgressView||cityProgress||%s||%s">%s</a>:</h3></td></tr>'
                % (countryName, city.getName(), city.getName())
            )
            html += "<tr><td>"

            if len(city.getCityObjects()) > 0:
                html += self._getProgressBar(
                    builtObjectCount * 100 / len(city.getCityObjects())
                )
            else:
            ...
            html += "</tr></td>"
            html += (
                "<tr><td>You have completed %s of %s constructions.</tr></td></table><br>"
                % (builtObjectCount, len(city.getCityObjects()))
            )
    ...
    def cityProgress(self, countryName, cityName):
        ...
        html += (
            "<p>You have completed the following constructions in %s:</p>"
            % city.getName()
        )
        ...
        for object in sorted(city.getCityObjects(), key=attrgetter("name")):
            if object.isBuilt():
                html += (
                    '<tr valign="middle"><td width="50" align="center"><img src="file:///%s" height="40"></td>'
                    % getImagePath(object.cityName, object.image)
                )
                html += '<td><a href="%s" %s>%s</a></td></tr>' % (
                    object.link,
                    getLinkColor(),
                    object.name,
                )
        ...
        html += (
            '<p><a href="ProgressView||countryProgress||%s">&lt;&lt; Back to country progress overview</a>'
            % (countryName)
        )
    ...
    def _getProgressBar(self, percentComplete):
        ...
        html += (
            '<td width="%s" bgcolor="#87CEFA">%s</td><td width="%s" bgcolor="#EEEEEE">%s</td>'
            % (2 * percentComplete, leftString, 200 - 2 * percentComplete, rightString)
        )
        html += "</tr></table>"
