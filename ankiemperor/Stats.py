...
class Stats(object):
    def __init__(self, db, eventManager):
        ...
        if (self.__todayStats is None) or (
            date.today() != self.__todayStats["statsDay"]
        ):
            self.__createNewStats()
    ...
    def isNewRecord(self):
        ...
        if (
            (self.__recordStats is not None)
            and (date.today() != self.__recordStats["statsDay"])
            and (self.getCardsAnsweredToday() > self.__recordStats["cardsAnswered"])
        ):
            self.__recordStats = self.__todayStats
            return True
        else:
